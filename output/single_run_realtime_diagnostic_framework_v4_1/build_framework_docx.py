#!/usr/bin/env python3
"""Render the reviewed TCPS-PA v4.1 Markdown framework into a polished DOCX.
...
OUTPUT = ROOT / "自动驾驶单次运行通用实时性—物理安全双向诊断框架.docx"
...
def configure_document(doc):
    ...
    p.paragraph_format.space_after = Pt(2)
    p.alignment = WD_ALIGN_PARAGRAPH.LEFT
    run = p.add_run("TCPS-PA v4.1  ·  单次运行通用实时性—物理安全双向诊断框架")
...
def add_cover(doc):
    ...
    r = kicker.add_run("METHOD REFERENCE  ·  TCPS-PA v4.1")
    ...
    r = title.add_run("自动驾驶单次运行通用实时性—物理安全\n双向诊断框架")
    set_run_font(r, size=28, color=INK, bold=True)

    subtitle = doc.add_paragraph()
    subtitle.alignment = WD_ALIGN_PARAGRAPH.CENTER
    subtitle.paragraph_format.space_after = Pt(28)
    r = subtitle.add_run("Four Event Times · Observability · Typed Component Contracts")
...
def render_markdown(doc, text):
    ...
    while idx < len(lines):
        ...
        if m:
            # Preserve the source number explicitly. Word's built-in List Number
            # otherwise continues numbering across unrelated lists and references.
            p = doc.add_paragraph()
...
def main():
    ...
    core.title = "自动驾驶单次运行通用实时性—物理安全双向诊断框架"
    core.subject = "Single-run event-and-loop temporal correctness and physical safety diagnosis"
    core.author = "TCPS-PA Method Working Draft"
    core.keywords = "Apollo, CARLA, real-time systems, temporal coherence, closed loop, architectural contract, dynamic deadline"
    doc.save(OUTPUT)
    print(OUTPUT)
