...
**版本：TCPS-PA v4.0（Single-Run Event-and-Loop Diagnostic Edition）**  
...
不能因为 timestamp 被改写成共同 header 就宣称 coherence 已成立。Temporal Coherence requirement \(\tau_C^{arch}\) 可以来自 Fusion/Planning 接口设计、传感同步规格或明确工程阈值；\(\tau_C^{phys}(x)\) 则应由跨源时间偏斜导致的相对状态误差包络反解。没有合格阈值时只报告 `COHERENCE_OBSERVED`，不得写 `INCOHERENCE_FAILURE`。
...
\[
\mathbf L^{obs}=[T_s,J_s,T_{feedback},T_{controller},G_{cmd}^{fresh},T_{actuation},\phi_{loop},H_{reuse}]
\]

tracking error、overshoot 和 oscillation 是与 \(\mathbf L\) 对齐的 physical/control behavior vector，不并入 `L` 后再循环证明。任何 composite loop verdict 都必须先逐子分量判定，再按预声明 required components 合成；缺失分量保持 `NOT_TESTABLE`。
...
合格来源优先级为：部署配置/接口契约/设计文档/经过确认的模块规范 > 独立测试标定的工程限制 > 明确标注的研究者分析阈值。仅从本 run 的中位数、最大值、默认日志频率或“Apollo 通常是 10 Hz”推断的阈值不得标记为 `QUALIFIED_ARCHITECTURAL`。若要求是 period，而观测量是 execution time 或 age，语义不兼容，不能直接比较。
...
\[
\mathbf S_{arch}=\boldsymbol\tau_{arch}^{req}-[R,A,G,C,L]^{obs}
\]

\[
\mathbf S_{phys}=\boldsymbol\tau_{phys}^{req}(x)-[R,A,G,C,L]^{obs}
\]
...
Physical dynamic contracts
  ...
  tau_A / tau_G / tau_C / tau_L if independently available
  per-component and composite observed verdict
...
| `temporal_coherence.csv` | `run_id,event_or_window_id,bundle_id,consumer_instance_id,input_id,input_role,source_time,source_semantics,read_time,raw_skew_ms,compensated_epoch,compensated_skew_ms,compensation_model,clock_error_ms,requirement_id,verdict,lineage_grade,missing_reason` |
...
| `architectural_contracts.csv` | `requirement_id,owner,artifact,version,scope_type,scope_id,dimension,metric_semantics,low,center,high,unit,mode_context,effective_time,lock_time,evidence_class,qualification` |
...
`P_ARCH_CONTRACT` 与 `P_PHYS_CONTRACT` 不得互相替代；`P_BUNDLE` 不通过时，Coherence 只能描述；`P_LOOP` 不通过时，Loop 结论最多是未限定窗口的 case-level observation；`P_EFFECT` 不稳定时，依赖 \(t_{phys}\) 的 C3_EVENT/C4_PHYS/C5 必须降级。
...
(U_E,U_L)
\rightarrow
\left(R,A,G,C,L\right)^{obs}
\parallel
\left(\boldsymbol\tau_{arch}^{req},\boldsymbol\tau_{phys}^{req}(x)\right)
}
\]
...
\[
\boxed{
\mathbf S_{arch}=\boldsymbol\tau_{arch}^{req}-\mathbf T^{obs},\quad
\mathbf S_{phys}=\boldsymbol\tau_{phys}^{req}(x)-\mathbf T^{obs}
}
\]
