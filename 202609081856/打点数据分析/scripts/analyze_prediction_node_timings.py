...
REPEATABLE_PHASES = {
    "cyber_taskmanager_notify_task_enter",
    "cyber_taskmanager_notify_task_exit",
    "cruise_mlp_inference_enter",
    "cruise_mlp_inference_exit",
    "model_cruise_mlp_enter",
    "model_cruise_mlp_exit",
}
ROLLUP_NODES = {
    "proc",
    "perception",
    ...
    try:
        connection.executescript(
            ...
            CREATE TABLE cadence_gaps (
                ...
                before_input_seq INTEGER,
                after_input_seq INTEGER,
...
def read_events(path: Path) -> tuple[dict[int, dict[str, dict[str, Any]]], dict[str, int], int, int]:
    ...
    with path.open(newline="", encoding="utf-8-sig") as handle:
        for row in csv.DictReader(handle):
            if not all(row.get(field) for field in ("trace_id", "phase", "event_id", "mono_ns", "tid")):
            ...
            phase_counts[phase] += 1
    return by_trace, dict(phase_counts), duplicate_event_ids, malformed_event_rows
...
def main() -> None:
    ...
    all_events, phase_counts, duplicate_event_ids, malformed_event_rows = read_events(events_path)
    ...
    required_core_phases = {"proc_enter", "proc_exit", "output_pub"}
    incomplete_core_traces = [
        {
            "trace_id": trace_id,
            "present_phases": sorted(trace),
            "missing_core_phases": sorted(required_core_phases - set(trace)),
        }
        for trace_id, trace in all_events.items()
        if not required_core_phases.issubset(trace)
    ]
    events = {
        trace_id: trace
        for trace_id, trace in all_events.items()
        if required_core_phases.issubset(trace)
    }
    if not events:
        raise RuntimeError("Prediction trace has no complete callback frames")
    ...
    for frame_index, trace_id in enumerate(ordered_trace_ids, start=1):
        ...
        if missing:
            missing_phases.append({"trace_id": trace_id, "missing": missing})
        ...
        frame = {
            ...
            "trace_id": trace_id,
            "proc_enter_tid": trace["proc_enter"]["tid"],
            "proc_exit_tid": trace["proc_exit"]["tid"],
            "output_pub_tid": trace["output_pub"]["tid"],
        ...
        anomaly_rows.append(
            {
                ...
                "proc_exit_tid": frame["proc_exit_tid"],
                "output_pub_tid": frame["output_pub_tid"],
    ...
    validation = {
        ...
        "status": (
            ...
            if any(
                ...
                if (
                    incomplete_core_traces
                    or malformed_event_rows
                    or context_missing_in
                    or context_missing_out
                )
                else "pass"
            )
        ),
        ...
        "counts": {
            "raw_trace_frames": len(all_events),
            "trace_frames": len(frame_rows),
            "skipped_incomplete_trace_frames": len(incomplete_core_traces),
        ...
        "quality_checks": {
            ...
            "incomplete_core_traces": incomplete_core_traces,
