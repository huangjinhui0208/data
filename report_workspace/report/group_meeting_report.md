# 《结果正确但响应过晚：Apollo闭环实时性对车速隐形Deadline和碰撞结局的影响》
...
实测 300 ms 控制延迟使端到端响应中位数从 300.358 ms 增加到 749.901 ms，增加 449.543 ms；响应阶段的墙钟速度积分距离 D_delay 从 5.284 m 增加到 12.716 m，增加 7.431 m；有效制动开始时的剩余净距 D2 从 33.490 m 降到 26.347 m，减少 7.143 m。baseline 组 0/7 碰撞，300 ms 主分析组 2/4 碰撞。数据支持“延迟→更晚的有效制动→更大距离债务→更小制动空间”的机制链。但当前样本不支持一个通用 700 ms 硬阈值；更准确的表述是，在本车速、起始净距和制动能力范围内，699–894 ms 落入当前样本的结局转换区。
...
SCB 的 `BRIDGE_CONFIG_LOADED`、`INITIALIZED`和 `APPLIED` 生命周期在 12 个 run 中均完整。300 ms 主分析组实测墙钟延迟中位数 300.124 ms，范围 300.047–300.318 ms；baseline 中位数 0.070 ms。`1031` 首个有效命令日志为 19.282 ms，其余 baseline 接近 0.07–0.09 ms。因此“注入 300 ms”已由独立墙钟日志验证，不是从目录名推断。
...
baseline 的 T_e2e 范围 299.655–399.750 ms，300 ms 主分析组为 699.155–893.870 ms，当前样本中两组范围完全分离。中位差 449.543 ms 大于注入的 300 ms，说明 t2 还受模块周期、消息相位和物理减速识别窗口影响。Control 输出到 t2 的组中位数，baseline 为 38.224 ms，300 ms 组为 447.421 ms，这段同时包含 SCB、Bridge/API 和动力学被识别为有效减速所需时间。

![组间端到端响应](../figures/group_e2e_response.png)

baseline 的 D_delay 范围 4.673–7.006 m，300 ms 主分析组为 11.749–15.451 m。组间中位差 7.431 m 是闭环响应变慢的直接空间代价。D_delay 的增加与 D2 的减少不完全等值，因为各 run 的 D1 也有数米波动。这说明不能只看延迟或只看碰撞率，必须把起始距离和制动能力一并放入预算。
...
完整停车 run 的 0 m 碰撞余量 M0=D1-D_delay-D_brake,data。baseline 中位数 3.471 m，范围 2.740–5.338 m；300 ms 组两个安全停车 run `1202`、`1211` 只剩 0.548 m 和 1.018 m。碰撞 run 没有完整制动端点，图中红色交叉仅标出碰撞和实测冲击速度，其纵坐标 0 是视觉边界，不是伪造的碰撞余量。

![当前样本的安全陡峭](../figures/safety_cliff.png)

若要求停车后仍保留 6 m，则 M6=M0-6。baseline 7/7 的 M6 都为负，中位数 -2.529 m。因此“baseline 无碰撞”只说明满足 0 m 几何接触边界，不代表满足 6 m 工程余量。若 6 m 是必须目标，当前起始距离与车速下即使不注入延迟也已不充足。
...
目标 Trace 的阶段时间由 monotonic clock 计算，通过 trace anchor 与 Fusion 源时刻对齐。源时刻到 Fusion 输出的组中位数，baseline 为 234.979 ms，300 ms 主分析组为 278.202 ms。Fusion→Prediction、Prediction→Planning STOP、Planning STOP→Control 大多为数毫秒到十余毫秒。ground detection 到 lidar detection 完成率组中位数为 0.976 和 0.971，处理耗时组中位数 92.399 和 92.184 ms。这不支持“整条功能链未运行”。
...
`1131` 的 T_e2e=799.636 ms，比同设置安全 run `1211` 慢 99.469 ms；D_delay=13.432 m，多 1.433 m；D2=24.826 m，少 3.376 m。CollisionSensor 和 actor history 记录到 7.988 m/s 的碰撞前车速。车辆确实制动，但制动开始时空间已被显著消耗。
...
`1131` 目标 Fusion 输出最大间隔 507.439 ms，碰撞时最后可用目标的源数据年龄 1006.892 ms，在相同 t1 相对时刻比 `1211` 老 600.163 ms。这不是目标从未出现，而是关键阶段更新变稀、源数据变老。同时 `1131` 的 D1 比 `1211` 小 1.944 m。因此定性应为“实时性主导候选的多因素碰撞”，不是 RT_ONLY_COLLISION：响应过晚是直接距离债务来源，Fusion 长空档和起始净距差异是必须保留的混杂因素。

![1131 Fusion时序与数据年龄](../figures/case_1131_fusion_timeline_age.png)

## 9. 案例 `1643`：更慢响应与更小起始净距共同作用

`1643` 的 T_e2e=893.870 ms，是主分析中最慢 run，比 `1211` 慢 193.703 ms。它的 D1=36.651 m，小 3.551 m；D_delay=15.451 m，多 3.451 m；D2=21.201 m，少 7.002 m。最终碰撞前车速 11.728 m/s，高于 `1131`，与更小剩余制动距离一致。
...
`1643` 最大 Fusion 输出间隔 115.906 ms，明显小于 `1131`，所以不应把两个碰撞强行归结为同一个 Fusion gap 模式。但碰撞时源数据年龄 599.668 ms，在同一相对时刻比 `1211` 老 309.285 ms。这说明“输出间隔”与“源数据年龄”必须分开：输出可以持续，但源帧仍可能变老。`1643` 同样是多因素碰撞，响应过晚、更小 D1 和当次制动能力共同决定了结局。

![1643 Fusion时序与数据年龄](../figures/case_1643_fusion_timeline_age.png)

## 10. 目标身份审计

两个碰撞 run 的 Planning STOP ID 与稳定 Fusion 目标一致。将 Apollo Fusion 目标与 CARLA actor history 的 `other` 行按墙钟时间对齐，并对 CARLA y 轴做坐标转换：`1131` 匹配 20 帧，二维位置误差中位数 1.689 m、p90 1.847 m；`1643` 匹配 20 帧，中位数 1.602 m、p90 1.921 m。两者都对应 CARLA actor 155，车型 `vehicle.lincoln.mkz_2020`，速度误差中位数为 0。考虑 Fusion 障碍物中心与 actor origin 的定义偏移，这支持“STOP 目标就是碰撞物体”的高置信身份链，但不宣称亚米级几何真值完全一致。
...
本节属 C 类模型结果，不是实测结局。模型将碰撞 run 的 t2 恢复为 baseline 中位响应 300.358 ms，使用该 run 在更早时刻实际观测的车速与 t1到恢复t2的墙钟积分；制动能力保持该碰撞 run 从实际t2到接触的能量等效减速度。它是“保持当次制动能力，只恢复响应时机”的局部反事实，不包含新闭环反馈和控制重算。

`1131` 回收 8.565 m，对观测接触边界的预测余量 +5.495 m，预测避碰。`1643` 回收 10.519 m，但预测余量仍为 -2.000 m，仍预测碰撞，冲击速度约 3.980 m/s。对 `1131`，恢复实时性在模型中足以改变结局；对 `1643`，它能减轻后果，但起始净距和当次制动能力使其仍越过接触边界。这正是不能用“小于 700 ms 就安全”替代距离预算的原因。

## 12. `1206`的数据质量冲突

`1206` 不缺 t1/t2 或主要模块日志。重算得 T_e2e=800.753 ms、D_delay=13.741 m、D2=26.049 m，且有近停与最小速度端点。若按稳定 Fusion 目标和 5.3074 m 偏移计算，停车端点投影净距 -1.627 m，是明显几何穿透；但没有 CARLA 碰撞事件，也没有 actor history 判断是传感器未触发、物理实体不同、目标漂移还是记录缺失。
...
原始数据表明，Apollo 的 Fusion、Prediction、Planning STOP 和 Control 链在碰撞 run 中并非整体消失；真正决定物理结局的是这些结果何时转化为车辆有效减速。实测 300 ms 延迟使 T_e2e 中位数增加约 449.5 ms，使 D_delay 增加约 7.43 m，并把 t2 剩余净距压缩约 7.14 m，与 baseline 0/7、300 ms 主分析组 2/4 碰撞的方向一致。
...
| 202607271131 | 300 ms | 是 | 799.636 | 38.258 | 13.432 | 24.826 | NA | NA | 碰撞，7.988 m/s |
...
| 202607271643 | 300 ms | 是 | 893.870 | 36.651 | 15.451 | 21.201 | NA | NA | 碰撞，11.728 m/s |
