...
自2026-08-10起，实际数据结果只有在无CARLA碰撞事件且最终端点满足 \(v_{\mathrm{final}}<0.01\ \mathrm{m/s}\) 时才判定为完整停车。对于满足该停车判据的run，当前兼容旧模型仍使用 \(t_2\) 之后最低速度样本作为经验制动完成端点：
...
自2026-08-10起，最终停车结局统一采用以下判据：对于没有CARLA碰撞事件的run，若统一最终端点的Localization实测速度满足

\[
v_{\mathrm{final}}<0.01\ \mathrm{m/s},
\]

则判定车辆已经停车。该判据使用严格小于号；\(v_{\mathrm{final}}=0.01\ \mathrm{m/s}\)或更高时不能标记为停车，只能标记为近停或停车未确认。\(v<0.1\ \mathrm{m/s}\)仍可作为近停诊断阈值，但不能单独作为最终停车结局。

以下202607201555数值按修改前的旧严格停车口径计算，仅用于说明最低速度代理可能包含停车后蠕行，不作为新的0.01 m/s停车判据结果：
...
- t_near_stop：首个 \(v<0.1\ \mathrm{m/s}\) 样本，仅作为近停诊断。
- v_final：统一最终端点的Localization实测速度。
- stop_data_observed：无CARLA碰撞事件且 \(v_{\mathrm{final}}<0.01\ \mathrm{m/s}\) 时为true，否则为false或停车未确认。
- t_stop_strict：\(v<0.01\ \mathrm{m/s}\) 持续至少0.5 s的起点。
...
8. 无CARLA碰撞事件且最终端点满足 \(v_{\mathrm{final}}<0.01\ \mathrm{m/s}\) 时，判定为完整停车并计算 \(D_{\mathrm{brake,data}}\) 和实际数据余量。
...
5. 未碰撞run终点使用报告统一的最低速度代理或严格停车端点，并保存终点实际速度；只有终点实测速度 \(v_{\mathrm{final}}<0.01\ \mathrm{m/s}\) 时才判定为停车。
