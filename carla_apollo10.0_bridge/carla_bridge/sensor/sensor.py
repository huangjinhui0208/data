...
class Sensor(Actor):
    ...
    def __init__(
        ...
        self._current_carla_frame_id = None
    ...
    def get_cyber_transform(self, pose, timestamp):  # pylint: disable=W0613
        ...
        if timestamp is None:
            transform.header.timestamp_sec = self.node.get_time()
        else:
            transform.header.timestamp_sec = timestamp
    ...
    def get_sensor_stamp(self, carla_sensor_data=None):
        if carla_sensor_data is not None and hasattr(carla_sensor_data, "frame"):
            return self.node.get_time(frame_id=carla_sensor_data.frame)
        if self._current_carla_frame_id is not None:
            return self.node.get_time(frame_id=self._current_carla_frame_id)
        return self.node.get_time()

    def get_sensor_msg_header(
        self,
        ...
        valid_sensor_timestamp_fields = {
            "camera_timestamp",
            "lidar_timestamp",
            "radar_timestamp",
        }
        if sensor_timestamp_field not in valid_sensor_timestamp_fields:
            raise ValueError(
                "Unsupported sensor timestamp field '{}'".format(
                    sensor_timestamp_field
                )
            )

        header = self.get_msg_header(
            frame_id=frame_id, timestamp=timestamp, time_frame_id=time_frame_id
        )
        setattr(header, sensor_timestamp_field, sensor_timestamp_ns)
        return header
    ...
    def update(self, frame, timestamp):
        self._current_carla_frame_id = frame
        try:
            if self.synchronous_mode:
