...
class Radar(Sensor):
    ...
    def __init__(self, uid, name, parent, relative_spawn_pose, node, carla_actor, synchronous_mode):
        ...
        self.radar_writer = node.new_writer(self.get_topic_prefix(), ContiRadar, qos_depth=2)
        self.listen()
    ...
    def sensor_data_updated(self, carla_radar_measurement):
        ...
        # stamp = self.get_sensor_stamp(carla_radar_measurement)
        stamp_ns = self.node.get_clock_ns(frame_id=carla_radar_measurement.frame)
        stamp = stamp_ns / 1000000000.0
        header = self.get_sensor_msg_header(
            sensor_timestamp_field="radar_timestamp",
            timestamp=stamp,
            time_frame_id=carla_radar_measurement.frame,
            sensor_timestamp_ns=stamp_ns,
        )
        header.frame_id = "radar_front"
        radar_msg = create_radar(
            header,
            radars,
        )
        self.radar_writer.write(radar_msg)
