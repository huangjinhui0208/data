...
class ActorFactory(object):
    ...
    def update_available_objects(self):
        ...
        for actor_id in new_actors:
            if self._is_collision_logger_sensor(actor_id):
                continue
    ...
    def _destroy_object(self, actor_id, delete_actor):
        ...
        if isinstance(actor, EgoVehicle) and getattr(self.node, "collision_sensor_logger", None):
            self.node.collision_sensor_logger.detach_from_ego(actor.uid)
        carla_actor = None
    ...
    def _create_object(self, uid, type_id, name, attach_to, spawn_pose, carla_actor=None):
        ...
        if type_id == ObjectSensor.get_blueprint_name():
            ...
            if carla_actor.attributes.get('role_name')\
                ...
                if getattr(self.node, "collision_sensor_logger", None):
                    self.node.collision_sensor_logger.attach_to(carla_actor)
            else:
        ...
        return actor
    def _is_collision_logger_sensor(self, actor_id):
        collision_sensor_logger = getattr(self.node, "collision_sensor_logger", None)
        return (
            collision_sensor_logger is not None
            and collision_sensor_logger.is_sensor_actor_id(actor_id)
        )